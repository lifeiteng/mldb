...
void skipCssWhitespace(ML::Parse_Context & context)
{
    context.skip_whitespace();
}

Utf8String expectElementType(ML::Parse_Context & context)
{
    return context.expect_text(" \n[:.#>+~{,*", false,
                               "expected CSS element type");
}

Utf8String expectIdName(ML::Parse_Context & context)
{
    return expectElementType(context);
}

Utf8String expectClassName(ML::Parse_Context & context)
{
    return expectElementType(context);
}

Utf8String expectAttrName(ML::Parse_Context & context)
{
    return expectElementType(context);
}

Utf8String expectAttrValue(ML::Parse_Context & context)
{
    return expectElementType(context);
}
...
PathElement
PathElement::
parse(ML::Parse_Context & context)
{
    PathElement result;
...
{
    ML::Parse_Context context(path.rawString(),
                              path.rawData(),
                              path.rawLength(),
                              1, 1);
...
{
    ML::Parse_Context context(filename.rawString(),
                              selector.rawData(),
                              selector.rawLength(),
...
std::shared_ptr<Selector>
parseSelectorSequence(ML::Parse_Context & context)
{
